...
void solve()
{
  ...
  {
      ll avg=-1;
      if((v[0].first+v[n-1].first)%2==0) avg=(v[0].first+v[n-1].first)/2;
      ll cnt=0;
      for(auto it:v)
      {
          if(it.first==avg) cnt++;
      ...
      else if((v[0].first+v[n-1].first)%2==0 && cnt==n-2)
      {
          ll avg=(v[0].first+v[n-1].first)/2;
          ll vol=avg-v[0].first;
          cout<<vol<<" ml. from cup #"<<v[0].second<<" to cup #"<<v[n-1].second<<"."<<endl;
      }
      else
      {
